...
> из отчета 1 "команда вращения по часовой" (слево), 2 - "команда вращения против часовой" (справо)

> а потом " в качестве кодировки направления вращения используется комбинация «01» (вращение по часовой стрелке). В случае, когда флажок «Reverse» (рисунок 2.60) установлен, комбинация меняется на «10» (вращение против часовой стрелки). "
...
> в отчете про номер мотора "Номер указывается в двоичной системе от 0 по 5" - что под 5?
...
Как было сказано ранее, если тросс один закручивается, то противоположный тросс на другом констроллере должен раскручиваться. И в классе ManualControl приведены следующие описания для решения этой задачи:
...
> Вопрос 16. Запуск команды sendADC влечет за собой непрерывное получение пакетов значений? Как остановить этот процесс? Отправить все биты опущенными?

> Вопрос 17. По итогу какой COM-порт используется для получения данных sendADC, так как есть разные подходы с и без "both"?

> Вопрос 18. Какая структура пакета у входящего пакета у исходящей команды sendADC? 
...
Обработка:
```
ema_convertion(data) Экспоненциальная скользящая средняя (EMA)
alpha = 0.01
